<style>
    ...
    @media only screen and (max-width: 600px) {
        .dt-init thead th:nth-child(1),
        .dt-init tbody td:nth-child(1),
        .dt-init thead th:nth-child(2),
        .dt-init tbody td:nth-child(2) {
            display: none;
        }
        img.img-thumbnail.product-thumb {
            width: 3rem;
...
<section class="py-4">
    <div class="container">
        <h3 class="fw-bolder text-center">Product List</h3>
        <center>
...
<script>
    $(function(){
        $('.dt-init').DataTable({
            ...
            serverSide: true,
            ajax: {
                url:"../classes/Master.php?f=dt_product_admin",
                method:"POST"
            },
            columns: [{
                    data: 'no',
                ...
                },
                {
                    ...
                    render:function(data, type, row, meta){
                        return '<img src="'+row.thumbnail+'" class="img-thumbnail product-thumb" />';
                    }
                },
                {
                    data: 'title',
                ...
                },
                {
                    ...
                    render:function(data, type, row, meta){
                        ...
                        switch(parseInt(row.status)){
                            case 0:
                                stat =  '<span class="badge bg-default border text-muted bg-gradient px-3 rounded-pill">Sold</span>' ;
                                break;
                            case 1:
                                stat =  '<span class="badge bg-primary bg-gradient px-3 rounded-pill">Availabe</span>' ;
                                break;
                            default:
                                stat = ""
                                break;
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        el.find('.edit_data,.delete_data,.view_data').attr('data-id',row.id).attr('data-name',row.title)
                        el.find('.edit_data').attr("href","./?page=products/manage_product&id="+row.id)
                        el.find('.view_data').attr("href","./?page=products/view_details&id="+row.id)
            ...
            drawCallback: function(settings) {
                ...
                $('.delete_data').click(function(){
                    _conf("Are you sure to delete <b>"+$(this).attr('data-name')+"</b> from list?","delete_product",[$(this).attr('data-id')])
                })
            },
            language:{
                oPaginate: {
